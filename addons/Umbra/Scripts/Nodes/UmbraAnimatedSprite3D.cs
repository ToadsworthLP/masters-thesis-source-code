...
[GlobalClass]
[Tool]
public partial class UmbraAnimatedSprite3D : Node3D, IUmbraCompanion
{
    ...
    [Export]
    public AnimatedSprite2D Target
    {
        get => target;
        set
        {
            AnimatedSprite2D previous = target;
            target = value;
    ...
    private AnimatedSprite2D target;
    private UmbraSpriteAxis spriteAxis = UmbraSpriteAxis.Z;
    private Texture2D normalMap;
    
    private AnimatedSprite3D visibleSprite;
    private AnimatedSprite3D shadowSprite;

    private StringName lastAnimation;
    private int lastFrame;

    public UmbraAnimatedSprite3D()
    {
#if TOOLS
    ...
    {
        EnsureChildrenExist();
        if(HasNode(VisibleSpriteInstanceName)) visibleSprite = GetNode<AnimatedSprite3D>(VisibleSpriteInstanceName);
        if(HasNode(ShadowSpriteInstanceName)) shadowSprite = GetNode<AnimatedSprite3D>(ShadowSpriteInstanceName);
    ...
    private void HandleTargetChanged(AnimatedSprite2D previous)
    {
        if (target != null)
        {
            UpdateSpriteProperties(visibleSprite);
    ...
    private void UpdateSpriteProperties(AnimatedSprite3D sprite)
    {
        sprite.PixelSize = 1.0f / root.PixelsPerMeter;

        sprite.SpriteFrames = target.SpriteFrames;
        sprite.Animation = target.Animation;
        sprite.Frame = target.Frame;
        sprite.SpeedScale = target.SpeedScale;
    ...
    private void UpdatePosition()
    {
        ...
        float targetHeight = target.SpriteFrames.GetFrameTexture(target.Animation, target.Frame).GetHeight();
        if (target.Centered)
        {
            if (SpriteAxis == UmbraSpriteAxis.Z)
            {
                position.Z += targetHeight / pixelsPerMeter / 2;
                position.Y += targetHeight / pixelsPerMeter / 2;
            }
        }
        else
        {
            position.Z += targetHeight / pixelsPerMeter;
        }

        Position = position;
    }

    private void UpdateTexture(AnimatedSprite3D sprite)
    {
        if(sprite.Animation == lastAnimation && sprite.Frame == lastFrame) return;
        
        ShaderMaterial material = (ShaderMaterial)sprite.MaterialOverride;
        material.SetShaderParameter("mainTexture", sprite.SpriteFrames.GetFrameTexture(sprite.Animation, sprite.Frame));
    ...
    private void EnsureChildrenExist()
    {
        if (!HasNode(VisibleSpriteInstanceName))
        {
            ...
            AnimatedSprite3D node = new AnimatedSprite3D();
    ...
    {
        if (target is AnimatedSprite2D sprite)
        {
            EnsureChildrenExist();
            Target = sprite;
        }
